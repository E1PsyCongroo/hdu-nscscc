...
/** IO bundle to interact with Issue slot
  ...
  *   number of wakeup ports for the slot
  */
class IssueSlotIO(val numWakeupPorts: Int)(implicit params: CoreParameters) extends Bundle {
...
/** Single issue slot. Holds a uop within the issue queue
  ...
  */
class IssueSlot(val numWakeupPorts: Int)(implicit params: CoreParameters) extends Module {
  ...
  valid := Mux(io.in_uop.valid, true.B, Mux(io.grant, false.B, valid))
