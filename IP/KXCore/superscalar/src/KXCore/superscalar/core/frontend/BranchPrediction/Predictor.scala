...
class BranchPrediction(implicit params: CoreParameters) extends Bundle {
  ...
  val predictedPC = Valid(UInt(params.commonParams.vaddrWidth.W))
...
class BranchPredictionBundle(implicit params: CoreParameters) extends Bundle {
  ...
  val preds = Vec(params.fetchWidth, new BranchPrediction)
}
...
class BranchPredictionBankUpdate(implicit p: Parameters) extends BoomBundle()(p) with HasBoomFrontendParameters {
  val is_mispredict_update = Bool()
  val is_repair_update     = Bool()
...
class BranchPredictionBankResponse(implicit p: Parameters) extends BoomBundle()(p) with HasBoomFrontendParameters {
...
abstract class BranchPredictorBank(implicit p: Parameters) extends BoomModule()(p) with HasBoomFrontendParameters {
  val metaSz  = 0
  def nInputs = 1
...
class BranchPredictor(implicit p: Parameters) extends BoomModule()(p) with HasBoomFrontendParameters {
  val io = IO(new Bundle {
  ...
  for (i <- 0 until nBanks) {
    banked_lhist_providers(i).io.f3_taken_br := banked_predictors(i).io.resp.f3.map(p => p.is_br && p.predicted_pc.valid && p.taken).reduce(_ || _)
  }
