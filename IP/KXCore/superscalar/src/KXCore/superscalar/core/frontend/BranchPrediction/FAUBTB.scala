...
class FAMicroBTBBranchPredictorBank(params: BoomFAMicroBTBParams = BoomFAMicroBTBParams())(implicit p: Parameters) extends BranchPredictorBank()(p) {
  ...
  s1_meta.write_way := Mux(s1_hits.reduce(_ || _), PriorityEncoder(s1_hit_ohs.map(_.asUInt).reduce(_ | _)), alloc_way)
  ...
  for (w <- 0 until bankWidth) {
    when(
      ...
      meta(s1_update_write_way)(w).ctr   := Mux(!s1_update_meta.hits(w), Mux(was_taken, 3.U, 0.U), bimWrite(meta(s1_update_write_way)(w).ctr, was_taken))
    }
  }
