...
// not compare estat
static const int DIFFTEST_NR_GREG =
    sizeof(arch_greg_state_t) / sizeof(uint32_t);
static const int DIFFTEST_NR_CSRREG =
    sizeof(arch_csr_state_t) / sizeof(uint32_t);
static const int DIFFTEST_NR_REG = DIFFTEST_NR_GREG + DIFFTEST_NR_CSRREG;

static const char *reg_name[DIFFTEST_NR_REG] = {
    "r0",      "ra",    "tp",     "sp",     "a0",     "a1",        "a2",
    "a3",      "a4",    "a5",     "a6",     "a7",     "t0",        "t1",
    "t2",      "t3",    "t4",     "t5",     "t6",     "t7",        "t8",
    " x",      "fp",    "s0",     "s1",     "s2",     "s3",        "s4",
    "s5",      "s6",    "s7",     "s8",     "crmd",   "prmd",      "euen",
    "ecfg",    "era",   "badv",   "eentry", "tlbidx", "tlbehi",    "tlbelo0",
    "tlbelo1", "asid",  "pgdl",   "pgdh",   "save0",  "save1",     "save2",
    "save3",   "tid",   "tcfg",   "tval",   "llbctl", "tlbrentry", "dmw0",
    "dmw1",    "estat", "this_pc"};
...
int Difftest::step(vluint64_t &main_time) {
  ...
  while (idx_commit < DIFFTEST_COMMIT_WIDTH && dut.commit[idx_commit].valid) {
    ...
    printf(
        "[%010ldns] mycpu : pc = %08x, inst = %08x, reg = %02d, val = %08x\n",
        main_time, dut.commit[idx_commit].pc, dut.commit[idx_commit].inst,
        dut.commit[idx_commit].wen ? dut.commit[idx_commit].wdest : 0,
        dut.commit[idx_commit].wdata);
#else
    printf("mycpu : pc = %08x, inst = %08x, reg = %02d, val = %08x\n",
           dut.commit[idx_commit].pc, dut.commit[idx_commit].inst,
           dut.commit[idx_commit].wen ? dut.commit[idx_commit].wdest : 0,
           dut.commit[idx_commit].wdata);
#endif
#endif
...
#else
    fprintf(trace_out,
            "mycpu : pc = %08x, inst = %08x, reg = %02d, val = %08x\n",
            dut.commit[idx_commit].pc, dut.commit[idx_commit].inst,
            dut.commit[idx_commit].wen ? dut.commit[idx_commit].wdest : 0,
            dut.commit[idx_commit].wdata);
#endif
  ...
  if (idx_commit == 0 && !dut.excp.excp_valid) {
    ...
    if (dead_clock_enable && dead_clock > DEAD_CLOCK_SIZE) {
      printf("CPU status no change for %d cycles, simulation must exist "
             "error!!!!\n",
             DEAD_CLOCK_SIZE);
      // fprintf(trace_out,"CPU status no change for %d cycles, simulation must
      // exist error!!!!\n", DEAD_CLOCK_SIZE);
  ...
  dead_clock_enable =
      dut.excp.excp_valid || dut.commit[idx_commit - 1].inst != 0x06488000u;
  dead_clock = 0;
#endif

#ifndef TRACE_COMP
  if (idx_commit > 0 && dut.commit[idx_commit - 1].pc == END_PC ||
      dut.excp.exceptionPC == END_PC) {
    sim_over = true;
  }
  return STATE_RUNNING;
#else
  ...
  if (idx_commit > 0)
    dut.csr.this_pc = dut.commit[idx_commit - 1].pc;
  ...
  if (idx_commit > 0 && dut.commit[idx_commit - 1].pc == END_PC ||
      dut.excp.excp_valid && dut.excp.exceptionPC == END_PC) {
    sim_over = true;
    return STATE_RUNNING;
  }
  ...
  if (dut.excp.excp_valid && dut.excp.exception == 0) {
    if (dut.excp.interrupt != 0) {
      ...
      // fprintf(trace_out,"warning: INT exception with no interrupt
      // detected\n");
    }
  }
  ...
  for (index = 0; index < idx_commit; index++) {
    if (dut.store[index].valid) {
      if (proxy->store_commit(dut.store[index].paddr, dut.store[index].data)) {
        printf("dut different at pc = 0x%08x, paddr = 0x%lx, vaddr = 0x%lx, "
               "data = 0x%lx\n",
               dut.commit[index].pc, dut.store[index].paddr,
               dut.store[index].vaddr, dut.store[index].data);
#ifdef SIMU_TRACE
        fprintf(trace_out,
                "dut different at pc = 0x%08x, paddr = 0x%lx, vaddr = 0x%lx, "
                "data = 0x%lx\n",
                dut.commit[index].pc, dut.store[index].paddr,
                dut.store[index].vaddr, dut.store[index].data);
#endif
  ...
  if (dut.excp.excp_valid) {
    if (dut.excp.exception != 0) { // not hard interrupt, nemu can detect itself
      ...
      // dut.excp.exceptionPC); fprintf(trace_out,"receive exception 0x%x at pc
  ...
  for (int i = 0; i < DIFFTEST_NR_CSRREG; i++) {
    if (!compare_mask[i])
      dut_regs_ptr[DIFFTEST_NR_GREG + i] = ref_regs_ptr[DIFFTEST_NR_GREG + i] =
          0;
  }

  /* compare */
  if (ecode_error ||
      memcmp(dut_regs_ptr, ref_regs_ptr,
             DIFFTEST_NR_REG * sizeof(uint32_t))) { // trace error print
    // display();
    for (int i = 0; i < DIFFTEST_NR_REG; i++) {
      if (dut_regs_ptr[i] != ref_regs_ptr[i]) {
        ...
        if (i < 32) {
          printf("%2s(r%2d) different at pc = 0x%08x, right= 0x%08x, wrong = "
                 "0x%08x\n",
                 reg_name[i], i, ref.csr.this_pc, ref_regs_ptr[i],
                 dut_regs_ptr[i]);
#ifdef SIMU_TRACE
          fprintf(trace_out,
                  "%2s(r%2d) different at pc = 0x%08x, right= 0x%08x, wrong = "
                  "0x%08x\n",
                  reg_name[i], i, ref.csr.this_pc, ref_regs_ptr[i],
                  dut_regs_ptr[i]);
#endif
          ...
          printf(
              "%7s different at pc = 0x%08x, right= 0x%08x, wrong = 0x%08x\n",
...
void Difftest::do_instr_commit(int i) {
  ...
  if (dut.commit[i].is_TLBFILL) {
    // printf("get a tlbfill inst from dut, give nemu an index:
  ...
  if (dut.commit[i].is_CNTinst) {
    // printf("rdcntv / rdcntid indt from dut, copy result to nemu:
    // %d\n",dut.commit[i].wdata);
    uint32_t timer_low, timer_high;
    timer_low = (uint32_t)((dut.commit[i].timer_64_value) & 0x00000000ffffffff);
    timer_high =
        (uint32_t)(((dut.commit[i].timer_64_value) & 0xffffffff00000000) >> 32);
    ...
    // printf("timer64: 0x%lx, low: 0x%x, high:
...
void Difftest::display() {
  ...
  fprintf(trace_out,
          "INDEX: 0x%08x, TLBEHI: 0x%08x, TLBELO0: 0x%08x, TLBELO1: 0x%08x\n",
          dut.csr.tlbidx, dut.csr.tlbehi, dut.csr.tlbelo0, dut.csr.tlbelo1);
  fprintf(trace_out,
          "ASID: 0x%08x, TLBRENTRY: 0x%08x, DMW0: 0x%08x, DMW1: 0x%08x\n",
          dut.csr.asid, dut.csr.tlbrentry, dut.csr.dmw0, dut.csr.dmw1);
  fprintf(trace_out, "*********************************************************"
                     "**********************\n");
#endif
